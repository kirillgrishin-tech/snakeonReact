...
function Records({prep,add,setAdd,setStyle,color,dispatch}){
    ...
    <div className='GameP' style={{color: color}}>
        ...
        {add === 0 ? 
            <button className = 'btn' style={{margin: '1em', color: 'blue'}} onClick={()=>{setStyle('fixed'); dispatch({type: 'init'});}}>Назад</button>:<button className = 'btn' style={{margin: '1em', color: 'blue'}} onClick={()=>{addRecord(value,add); setAdd(0);}}>Добавить</button>}
    </div>
    )
}
